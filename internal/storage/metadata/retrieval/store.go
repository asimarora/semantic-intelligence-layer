...
func matches(filter Query, document retrievalcontracts.Document) bool {
	...
	if requestID := strings.TrimSpace(filter.RequestID); requestID != "" && !strings.EqualFold(strings.TrimSpace(document.RequestID), requestID) {
		return false
	}
	if status := strings.TrimSpace(filter.Status); status != "" && !strings.EqualFold(strings.TrimSpace(document.Status), status) {
		return false
	}
	if outcome := strings.TrimSpace(filter.Outcome); outcome != "" && !strings.EqualFold(strings.TrimSpace(document.Outcome), outcome) {
		return false
	}
...
func normalizeDocument(document retrievalcontracts.Document) retrievalcontracts.Document {
	...
	document.RequestID = strings.TrimSpace(document.RequestID)
	document.Status = strings.TrimSpace(document.Status)
	document.Outcome = strings.TrimSpace(document.Outcome)
	...
	document.PolicyName = strings.TrimSpace(document.PolicyName)
	document.RejectReason = strings.TrimSpace(document.RejectReason)
