...
type Store interface {
	...
	Replay(context.Context, string) ([]rawevidence.Record, error)
}
...
func (store *FileStore) Replay(ctx context.Context, source string) ([]rawevidence.Record, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	if err := ctx.Err(); err != nil {
	...
	path := filepath.Join(store.root, sourceFileName(source))
	data, err := os.ReadFile(path)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil
		}
		return nil, fmt.Errorf("read raw storage file %s: %w", path, err)
	}

	lines := strings.Split(strings.TrimSpace(string(data)), "\n")
	records := make([]rawevidence.Record, 0, len(lines))
	for _, line := range lines {
		...
		var record rawevidence.Record
		if err := json.Unmarshal([]byte(line), &record); err != nil {
			return nil, fmt.Errorf("decode raw record from %s: %w", path, err)
		}
		if err := record.Validate(); err != nil {
			return nil, fmt.Errorf("validate raw record from %s: %w", path, err)
		}
...
func (store *MemoryStore) Replay(ctx context.Context, source string) ([]rawevidence.Record, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	if err := ctx.Err(); err != nil {
	...
	records := make([]rawevidence.Record, 0, len(store.records))
	for _, record := range store.records {
		if source != "" && !strings.EqualFold(strings.TrimSpace(record.Source), source) {
			continue
		}
		records = append(records, cloneRecord(record))
	}
