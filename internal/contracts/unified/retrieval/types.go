...
const (
	...
	EntityTypeNetworkAccess  = "network_access"
)
...
func (document Document) Validate() error {
	...
	switch strings.TrimSpace(document.EntityType) {
	case EntityTypeNetworkSession:
		if strings.TrimSpace(document.SessionID) == "" {
			return fmt.Errorf("session_id is required for network_session")
		}
		if strings.TrimSpace(document.Status) == "" {
			return fmt.Errorf("status is required for network_session")
		}
	case EntityTypeNetworkAccess:
		if strings.TrimSpace(document.RequestID) == "" {
			return fmt.Errorf("request_id is required for network_access")
		}
		if strings.TrimSpace(document.Outcome) == "" {
			return fmt.Errorf("outcome is required for network_access")
		}
	default:
		return fmt.Errorf("unsupported entity_type %q", document.EntityType)
	}
