...
func (service Service) Run(ctx context.Context, inputDir string) (Result, error) {
	...
	logger := service.logger().With("source", access.Source, "input", inputDir, "files", len(files))
	...
	for _, relPath := range files {
		...
		adapted, err := access.Adapt(service.Source, payload, time.Now().UTC())
		if err != nil {
		...
		logger.Debug("ingested access evidence", "path", relPath, "event_id", adapted.Record.EventID, "source_key", adapted.Record.SourceKey)
	}

	logger.Info("ingest completed", "files", result.Files, "persisted", result.Persisted, "published", result.Published)
	return result, nil
}
