...
func (service *Service) RunCycle(ctx context.Context, now time.Time) (Result, error) {
	...
	for _, tenantID := range service.config.Tenants {
		...
		candidates, err = service.enrichCandidatesWithAccess(ctx, candidates, from, now)
		if err != nil {
			errs = append(errs, fmt.Errorf("enrich watcher candidates for tenant %s: %w", tenantID, err))
		}
...
func (service *Service) detectCandidates(tenantID string, events []unifiedsessions.Event) []candidate {
	...
	timelines := make(map[string][]unifiedsessions.Event)
	for _, event := range events {
		subscriberID := strings.TrimSpace(event.SubscriberID)
		if subscriberID == "" {
			continue
		}
		timelines[subscriberID] = append(timelines[subscriberID], event)
		if !qualifiesForRepeatedDisconnect(event, service.config.ShortSessionThreshold) {
			continue
		}

		existing, ok := grouped[subscriberID]
		if !ok {
			grouped[subscriberID] = candidate{
				TenantID:   tenantID,
	...
	for _, candidate := range grouped {
		...
		candidate.SessionEvents = append([]unifiedsessions.Event(nil), timelines[candidate.Subscriber]...)
...
func (service *Service) enrichCandidatesWithAccess(ctx context.Context, candidates []candidate, from, to time.Time) ([]candidate, error) {
	if service == nil || service.access == nil || len(candidates) == 0 {
	...
	for index := range candidates {
		accessEvents, err := service.access.SearchNetworkAccessEvents(ctx, accessstore.Query{
			TenantID:     candidates[index].TenantID,
			SubscriberID: candidates[index].Subscriber,
			From:         &from,
			To:           &to,
			Limit:        sessionquery.MaxAccessLimit,
		})
		if err != nil {
			errs = append(errs, fmt.Errorf("search recent access events for subscriber %s: %w", candidates[index].Subscriber, err))
			continue
		}
		candidates[index].AccessEvents = accessEvents
		candidates[index].CorrelationSummary = agentharness.SummarizeCrossSourceCorrelation(accessEvents, candidates[index].SessionEvents)
	}
...
func buildRunRequest(candidate candidate, now time.Time, window time.Duration) agenttypes.RunRequest {
	...
	queryParts := buildWatcherQueryParts(candidate)
...
func sortCandidates(candidates []candidate) {
	sort.Slice(candidates, func(left, right int) bool {
		leftCorrelated := strings.TrimSpace(candidates[left].CorrelationSummary) != ""
		rightCorrelated := strings.TrimSpace(candidates[right].CorrelationSummary) != ""
		if leftCorrelated != rightCorrelated {
			return leftCorrelated
		}

		leftHasAccess := len(candidates[left].AccessEvents) > 0
		rightHasAccess := len(candidates[right].AccessEvents) > 0
		if leftHasAccess != rightHasAccess {
			return leftHasAccess
		}
...
func watcherGoal(candidate candidate) string {
	switch {
	case candidateHasAccessOutcome(candidate.AccessEvents, unifiedaccess.OutcomeRejected):
		return fmt.Sprintf("Did %s fail auth before connecting?", candidate.Subscriber)
	case strings.TrimSpace(candidate.CorrelationSummary) != "":
		return fmt.Sprintf("Investigate correlated access and repeated short-session disconnects for subscriber %s", candidate.Subscriber)
	case len(candidate.AccessEvents) > 0:
		return fmt.Sprintf("Investigate access context around repeated short-session disconnects for subscriber %s", candidate.Subscriber)
	default:
		return fmt.Sprintf("Investigate repeated short-session disconnects for subscriber %s", candidate.Subscriber)
	}
}

func buildWatcherQueryParts(candidate candidate) []string {
	...
	add("repeated short session disconnect")
	add("subscriber " + candidate.Subscriber)
	add("stop")
	if len(candidate.AccessEvents) > 0 || strings.TrimSpace(candidate.CorrelationSummary) != "" {
		add("access authentication")
	}
	if candidateHasAccessOutcome(candidate.AccessEvents, unifiedaccess.OutcomeRejected) || candidateHasAccessOutcome(candidate.AccessEvents, unifiedaccess.OutcomeChallenged) {
		add("auth failure")
	}
	if candidateHasAccessOutcome(candidate.AccessEvents, unifiedaccess.OutcomeRejected) {
		add(string(unifiedaccess.OutcomeRejected))
	}
	if candidateHasAccessOutcome(candidate.AccessEvents, unifiedaccess.OutcomeChallenged) {
		add(string(unifiedaccess.OutcomeChallenged))
	}
	if candidateHasAccessOutcome(candidate.AccessEvents, unifiedaccess.OutcomeAccepted) {
		add(string(unifiedaccess.OutcomeAccepted))
	}
	if strings.TrimSpace(candidate.CorrelationSummary) != "" {
		add("correlated")
	}
	if candidate.Latest.NASIPAddress != "" {
		add("nas " + candidate.Latest.NASIPAddress)
	}
	return parts
}

func candidateHasAccessOutcome(events []unifiedaccess.Event, outcome unifiedaccess.Outcome) bool {
	for _, event := range events {
