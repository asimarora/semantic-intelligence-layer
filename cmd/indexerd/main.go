...
func main() {
        ...
        if err != nil {
                fmt.Fprintf(os.Stderr, "indexerd bootstrap failed: %v\n", err)
                os.Exit(1)
        }

        logRuntime, err := sillogging.New(cfg.App, cfg.Logging, "indexerd", os.Stderr)
        if err != nil {
                fmt.Fprintf(os.Stderr, "indexerd logger failed: %v\n", err)
                os.Exit(1)
        }

        logRuntime.Logger.Info("service scaffold ready")
        if err := logRuntime.Close(); err != nil {
                fmt.Fprintf(os.Stderr, "indexerd logger shutdown failed: %v\n", err)
                os.Exit(1)
        }
}
